...
#define CONFIG_PRINT_MEASUREMENTS_USB_CDC                                      \
  1    // Set to `1` if we want serial (USB-to-Serial) printout for every
       // measurement.
#endif // CONFIG_PRINT_MEASUREMENTS_USB_CDC

#ifndef CONFIG_PRINT_MEASUREMENTS_UART_GPIO
#define CONFIG_PRINT_MEASUREMENTS_UART_GPIO                                    \
  1    // Set to `1` if we want serial (TX GPIO) printout for every measurement.
#endif // CONFIG_PRINT_MEASUREMENTS_UART_GPIO

#ifndef CONFIG_ENABLE_WIFI
#define CONFIG_ENABLE_WIFI                                                     \
  1 // Set to `1` if we want to use WiFi (ie not requred for Serial only
    // application)
#endif

#define CALIBRATION_POINT_SAMPLE_COUNT                                         \
  32 // Number of ADC samples to take for new calibration point
#define CALIBRATION_POINT_ACCURACY_POINT                                       \
  50 // We expect all CALIBRATION_POINT_SAMPLE_COUNT samples to be
     // within CALIBRATION_POINT_ACCURACY_POINT of each other
     // in order to have successful calibration point creation

#define CALIBRATION_BOUNDARY_ITEMS                                             \
  2 // We need lower and upper boundary item for safety
...
#define ADC_MIN_EQUALS_MM                                                      \
  5.0f // ADC_MIN should be considered this diameter (in mm)
#define ADC_MAX                                                                \
  4095 // Maximum value we can get from our ADC (should be 2^n-bits)
#define ADC_MAX_EQUALS_MM                                                      \
  0.0f // ADC_MAX should be considered this diameter (in mm)
...
#define ADC_MAX_SAMPLES                                                        \
  64 // Maximum number of ADC samples we can take in one measurement cycle
#define ADC_SAMPLES_PER_MEASUREMENT_CYCLE                                      \
  16 // How many samples to take for each measurement cycle
...
// #define ADC_CHIP_MCP3021                            // Specify which ADC chip
// we are using
#define ADC_CHIP_INTERNAL // Use internal ADC

#ifdef ADC_CHIP_INTERNAL
#define ADC_PIN A0
#endif
...
#define LED_MEASUREMENT_ON()                                                   \
  do {                                                                         \
    digitalWrite(LED_MEASUREMENT_PIN, LOW);                                    \
  } while (0)
#define LED_MEASUREMENT_OFF()                                                  \
  do {                                                                         \
    digitalWrite(LED_MEASUREMENT_PIN, HIGH);                                   \
  } while (0)
#define LED_SERIAL_ON()                                                        \
  do {                                                                         \
    digitalWrite(LED_SERIAL_PIN, LOW);                                         \
  } while (0)
#define LED_SERIAL_OFF()                                                       \
  do {                                                                         \
...
#define EEPROM_MAGIC_KEY                                                       \
  0x53 // Predefined value used to verify EEPROM has been initialized
...
// Calibration struct -> holds all calibration points and total number of valid
// calibration points
